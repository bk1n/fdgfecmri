...
\cite{moriceEndometrialCancer2016}. 
Morphological signs of nodal involvement, including size criteria, on CT and MRI vary and have been found to be unreliable markers of malignancy
...
Reports on the utility of SUVmax in the detection of nodal involvement are limited and most studies to date have used visual assessment of the level of uptake in nodes compared to background.
...
\section{Methods}
    ...
    SUVmax normalised to SA diameter (SNSA) was calculated by dividing the SUVmax by the SA diameter of each node for FDG-PET/CT and FEC-PET/CT.
    ...
    Additionally, due to strong correlation between features, calculated variables (NTR, STAR, SNSA) were excluded as features. 
    ...
    Receiver operating characteristic (ROC) analysis was applied, using the 'pROC' package in R, for each quantitative measure to generate ROC curves. 
...
\section{Results}
    ...
    The demographics of patients eligible for analysis are summarised in Supplementary Table \ref{table:demographics}. A total of 99 patients were eligible for analysis: 33 cervical cancer patients and 66 endometrial cancer patients. 67 and 73 patients received left pelvic and right pelvic lymphadenectomy, respectively, while just 26 patients underwent para-aortic lymphadenectomy. 88\% of cervical cancer patients were diagnosed as FIGO stage IB1 and no patients in this cohort had a FIGO stage greater than II. In the endometrial cancer cohort, FIGO stage ranged from IA to IVB.

    \subsection{Quantitative measures in benign and malignant lymph nodes}
    \begin{figure}[h]
        \centering
        \includegraphics[width=\textwidth]{../figures/FDG_FEC_ADC_byRegion_byCancer_combined}
        \caption{
            (A) Quantitative measures of measured lymph nodes of each region. One-way ANOVA with Tukey's post-hoc test was used to determine significance between regions. (B) Quantitative measures of measures lymph nodes in endometrial cancer and cervical cancer patients. Mann-Whitney U-test was used to determine significance between endometrial and cervical cancer. FDG-PET/CT (left), FEC-PET/CT (middle), and DW-MRI (right).}
        \label{fig:quant_byRegion_byCancer}
    \end{figure}

    % overview of FDG... byCancer 
    % overview of FDG... byRegion
    % rationale for splitting / merging
    Raw quantitative measures from the three imaging modalities were compared between pelvic and para-aortic regions and between endometrial and cervical cancer. No significant difference was found between regions in SUVmax or ADCmean on FDG-PET/CT and DW-MRI (Figure \ref{fig:quant_byRegion_byCancer}A). Lymph nodes of the para-aortic region on FEC-PET/CT had significantly higher SUVmax than left and right pelvis, a trend that was consistent across all three imaging modalities. Interestingly, the average SUVmax of nodes on FDG-PET/CT was significantly higher in endometrial than in cervical cancer (Figure \ref{fig:quant_byRegion_byCancer}B).

    % overview of quants in endo
    Quantitative measures, both raw and calculated, were compared in benign and malignant lymph nodes in endometrial cancer. All measures were significantly higher in malignant than benign lymph nodes on FDG-PET/CT (Figure \ref{fig:quant_inEndo}A). Short-axis diameter, SUVmax, and STAR were all significantly elevated in malignant lymph nodes on FEC-PET/CT (Figure \ref{fig:quant_inEndo}B). ADCmean and ADCmean NTR was not significantly different between benign and malignant lymph nodes in endometrial cancer (Figure \ref{fig:quant_inEndo}C).

    % overview of quants in cer
    No calculated or raw measures were significantly different between benign and malignant nodes on FDG-PET/CT and FEC-PET/CT in cervical cancer, although average raw measures were consistently higher in malignant nodes (\ref{fig:quant_inCer}A \& \ref{fig:quant_inCer}B). Low numbers of measured nodes in cervical cancer may limit the power of statistical analysis. Interestingly, ADCmean and ADCmean NTR were significantly lower in malignant nodes in cervical cancer (\ref{fig:quant_inCer}C). 
    % should the low numbers bit above be in the discussion??

    \begin{figure}[h]
        \centering
        \includegraphics[width=\textwidth]{../figures/fdg_fec_inEndo}
        \caption{
            Raw and calculated quantitative measures from benign (B) and malignant (M) lymph nodes in endometrial cancer. 
            Nodes of all regions are pooled together for this analysis.
            Red lines represent optimal cutpoints for F1 score maximisation.
            Significance was tested between groups using Mann-Whitney U-test.
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=\textwidth]{../figures/fdg_fec_inCer}
        \caption{
            Raw and calculated quantitative measures from benign (B) and malignant (M) lymph nodes in cervical cancer.  
            Nodes of all regions are pooled together for this analysis. 
            Significance was tested between groups using Mann-Whitney U-test. 
            *, p $<$ 0.05.}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.75\textwidth]{../figures/pairwise_corr_raw}
    ...
    Previous literature has shown elevated primary tumour SUVmax and depressed ADCmean in patients presenting with malignant lymph nodes \cite{antonsenSUVmax18FDGPET2013,husbyMetabolicTumorVolume2015,moharamzadDiagnosticPerformanceApparent2021}. SUVmax of the primary tumour was significantly elevated on FDG-PET/CT but not on FEC-PET/CT, with no significant difference found on DW-MRI (Supplementary Figure \ref{fig:quants_pt_byCanc}). No significant difference in any quantitative measure was found in primary tumours of the cervix.

    \FloatBarrier
    \subsection{Diagnostic performance of quantitative measures}
    % overview of diagnostic performance of quants
        % table!
    Diagnostic performance of each measure, along with optimal cutpoints, in endometrial cancer is summarised in Figure \ref{fig:all_performance}. 

    \begin{figure}[h]
        \centering
        \includegraphics[width=0.75\textwidth]{../figures/AllPerformance_byMeasure}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.75\textwidth]{../figures/ROC_byMeasure}
    ...
    No raw quantitative measure clearly outperformed the others on FDG-PET/CT, as shown by the ROC curves in Figure \ref{fig:roc_curves}. SUVmax from FDG-PET/CT showed relatively high sensitivity and specificity, with the highest AUC of the quantitative measures. Short-axis diameter had the highest sensitivity of all measures from FDG-PET/CT.
    ...
    Logistic LASSO regression shrunk all coefficients of features, excluding SUVmax and SA diameter, from FDG-PET/CT to zero and exhibited relatively poor diagnostic performance (sensitivity 0.70, specificity 0.79), likely due to the sparsity of data available for training. Univariate logistic regression models fit on SUVmax (sensitivity 0.67, specificity 0.87, AUC 0.87) and SA diameter (sensitivity 0.74, specificity 0.80, AUC 0.83) from FDG-PET/CT narrowly outperformed LASSO (Figure \ref{fig:all_performance}). The logistic regression model trained on SUVmax performed relatively well compared to raw and calculated quantitative measures alone. The predicted probability of malignancy based on SUVmax from FDG-PET/CT is shown in Figure \ref{fig:lr_model_probability}. 

    \begin{figure}[h]
        \centering
        \includegraphics[width=0.5\textwidth]{../figures/lr.model_plt_endo_SUV.png}
