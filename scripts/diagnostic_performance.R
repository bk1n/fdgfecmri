...
plt_df_tidy = plt_df %>%
  ...
  mutate(measure_type = factor(measure_type, levels = c('FDG-PET/CT', 'FEC-PET/CT', 'DW-MRI')),
         name = factor(name, levels = c('FDG SUVmax', 'FDG STAR', 'FDG SUVmax NTR',
...
mcnemar_test = function(measure, data) {
  ...
  pos_cm = table(quant = (pos$q == pos$y),
                 vis = (pos$v == pos$y))
  
  neg_cm = table(quant = (neg$q == neg$y),
                 vis = (neg$v == neg$y))
