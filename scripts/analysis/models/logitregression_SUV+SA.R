...
find_opt_cutoff_LR = function(x){
  ...
  lr.model = glm(HIST ~ FDG_SUV + FDG_SA, 
                    lr_train,
                    family = binomial())
...
fdg_lr = fdg %>%
  ...
  filter(!is.na(FDG_SUV) & !is.na(FDG_SA)) %>%
  select(HIST, FDG_SUV, FDG_SA)
...
write.csv(res.model.avg, './figures/tables/lr.model_optCutOff_SUV+SA.csv')
write.csv(res.model, './figures/tables/lr.model_optCutOff_fullRes_SUV+SA.csv')
...
write.csv(res.model.avg, './figures/tables/lr.model_performance_SUV+SA.csv')
write.csv(res.model, './figures/tables/lr.model_fullRes_SUV+SA.csv')

res.model.avg = read_csv('./figures/tables/lr.model_performance_SUV+SA.csv')
opt_cutoff = read_csv('./figures/tables/lr.model_optCutOff_SUV+SA.csv')

lr.model.full = glm(HIST ~ FDG_SUV + FDG_SA, 
                    fdg_lr,
                    family = binomial())
...
write.csv(lr, './figures/tables/lr_roc_results_SUV+SA.csv', row.names = F)

## plot logistic regression
FDG_SUV = seq(0, to = 30, by = 0.1)
x = data.frame(FDG_SUV)
...
annot = data.frame(x = rep(15,6),
                   ...
                   label = c(
                     ...
                     paste0('Optimal SUV cutoff: ', opt_cutoff$optimal_SUVcutoff),
...
ggsave('./figures/lr.model_plt_endo_SUV+SA.png',
       g_lr,
       width = 5,
       height = 5,
       units = 'in',
