...
data_clean <- data %>%
  ...
  mutate(
    FDG_SUV_RP_CR = rowMeans(select(., FDG_SUV_RP_1_CR, FDG_SUV_RP_2_CR), na.rm = T),
  ...
  mutate(
    FEC_SUV_RP_CR = rowMeans(select(., FEC_SUV_RP_1_CR, FEC_SUV_RP_2_CR), na.rm = T),
  ...
  mutate(
    ADC_PT_CR = rowMeans(select(., ADC_PT_1_CR, ADC_PT_2_CR), na.rm = T),
  ...
  mutate(
    FDG_SUV_RP = case_when(
      is.na(FDG_SUV_RP_CR) ~ FDG_SUV_RP,
      TRUE ~ FDG_SUV_RP_CR
    ),
    FDG_SUV_LP = case_when(
      is.na(FDG_SUV_LP_CR) ~ FDG_SUV_LP,
      TRUE ~ FDG_SUV_LP_CR
    ),
    FDG_SUV_PALN = case_when(
      is.na(FDG_SUV_PALN_CR) ~ FDG_SUV_PALN,
  ...
  mutate(
    FEC_SUV_RP = case_when(
      is.na(FEC_SUV_RP_CR) ~ FEC_SUV_RP,
      TRUE ~ FEC_SUV_RP_CR
    ),
    FEC_SUV_LP = case_when(
      is.na(FEC_SUV_LP_CR) ~ FEC_SUV_LP,
      TRUE ~ FEC_SUV_LP_CR
    ),
    FEC_SUV_PALN = case_when(
      is.na(FEC_SUV_PALN_CR) ~ FEC_SUV_PALN,
...
clin_data <- clin_data %>%
  ...
  mutate(ENDO_HIST = case_when(
    ENDO_HIST_ENDO == T ~ "endo",
